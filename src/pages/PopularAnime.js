    ...
    <div>
      ...
      {!loading && (
        <Parent>
          ...
          <CardWrapper>
            {items.map((item) => (
              <Links key={item.id} to={"/id/" + item.id}>
                <img src={item.coverImage.large} alt={item.title.english || item.title.userPreferred || "Anime Poster"} />
          ...
          <NavButtons>
            {Number(page) > 1 && (
              <NavButton to={"/popular/" + (Number(page) - 1)}>
                Previous
              </NavButton>
            )}
            <NavButton to={"/popular/" + (Number(page) + 1)}>Next</NavButton>
          </NavButtons>
        </Parent>
      )}
    </div>
